...
class ControlDeMatrimoniosController extends Controller
{
    ...
    public function archivarAdjunto(Request $request)
    {
        try {
            ...
            $pathSource = Storage::disk('fichas-matri')->getDriver()->getAdapter()->applyPathPrefix($nombreArchivo);
            $destinationPath = Storage::disk('fichas-matri')->getDriver()->getAdapter()->applyPathPrefix('archivado/' . $nombreArchivo);
            ...
            $existeArchivo = intval(Storage::disk('fichas')->exists('matri/archivado/' . $nombreArchivo));
