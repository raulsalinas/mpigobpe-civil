...
class ControlMatrimonioView {
    ...
    obtenerMatrimonio = () => {
        ...
        if ((idByURL) != null && idByURL > 0) {
            this.model.cargarDatosMatrimonio(idByURL).then((respuesta) => {
                ...
                $('[name=ape_mar]').val(respuesta.ape_mar);
                $('[name=nom_mar]').val(respuesta.nom_mar);
                // $('[name=ubigeo1]').val(respuesta.ubigeo1);
                $('[name="ubigeo1"]').val(respuesta.ubigeo1).trigger('change')

                $('[name=ape_esp]').val(respuesta.ape_esp);
                $('[name=nom_esp]').val(respuesta.nom_esp);
                // $('[name=ubigeo2]').val(respuesta.ubigeo2);
                $('[name="ubigeo2"]').val(respuesta.ubigeo2).trigger('change')
                $('[name=fch_cel]').val(respuesta.fch_cel);
     ...
     */
        ...
        $('#modalRecibo').on("click", "button.continuar", (e) => {
            ...
            const idRegistro = document.querySelector("form[id='controlMatrimoniosForm'] input[name='id']").value;
            ...
            abrirPestañaVisualizarAdjunto(2,idRegistro,0);
